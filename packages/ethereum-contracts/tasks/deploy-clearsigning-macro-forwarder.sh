...
# Usage:
# tasks/deploy-clearsigning-macro-forwarder.sh <network>
#
# The invoking account needs to be (co-)owner of the resolver and governance
#
# important ENV vars:
# RELEASE_VERSION, CLEARSIGNINGMACROFWD_DEPLOYER_PK
...
deployerPk=${CLEARSIGNINGMACROFWD_DEPLOYER_PK:-${ONLY712MACROFWD_DEPLOYER_PK:-}}

tmpfile="/tmp/$(basename "$0").addr"

# deploy
DETERMINISTIC_DEPLOYER_PK=$deployerPk npx truffle exec --network "$network" ops-scripts/deploy-deterministically.js : ClearSigningMacroForwarder | tee "$tmpfile"
...
# allow to fail
set +e
npx truffle run --network "$network" verify ClearSigningMacroForwarder@"$contractAddr"
set -e

# set resolver
ALLOW_UPDATE=1 npx truffle exec --network "$network" ops-scripts/resolver-set-key-value.js : ClearSigningMacroForwarder "$contractAddr"
