...
contract GovAction is Script {
    ...
    function _executeBatchChangeSuperTokenAdmin(BaseConfig memory baseConfig) internal {
        // For batch operations, we'll use space-separated values (easier to parse)
        // The shell script should pass them as space-separated
        string memory tokensStr = vm.envOr("TOKEN_ADDRESSES", string(""));
        require(bytes(tokensStr).length > 0, "TOKEN_ADDRESSES must be set (space-separated)");
        
        string memory adminsStr = vm.envOr("NEW_ADMINS", string(""));
        require(bytes(adminsStr).length > 0, "NEW_ADMINS must be set (space-separated)");

        // Parse space-separated addresses using vm.parseJson for arrays
        // For now, we'll use a simpler approach: expect JSON arrays
