...
contract VestingSchedulerV3Tests is FoundrySuperfluidTester {
    ...
    function _createVestingScheduleWithDefaultData(address sender, address receiver) private {
        ...
        vestingScheduler.createVestingSchedule(
            superToken, receiver, START_DATE, CLIFF_DATE, FLOW_RATE, CLIFF_TRANSFER_AMOUNT, END_DATE, 0, EMPTY_CTX
        );
        vm.stopPrank();
    }
    ...
    function test_vesting_scheduler_is_superapp() public {
        assertTrue(sf.host.isApp(ISuperApp(address(new VestingSchedulerV3(sf.host)))));
    }
    ...
    function testCreateVestingSchedule() public {
        ...
        vm.startPrank(alice);
        vestingScheduler.createVestingSchedule(
            superToken, bob, START_DATE, CLIFF_DATE, FLOW_RATE, CLIFF_TRANSFER_AMOUNT, END_DATE, EMPTY_CTX
        );
        vm.stopPrank();
    ...
    function testUpdateVestingSchedule() public {
        ...
        vm.stopPrank();
        vm.startPrank(alice);
        vestingScheduler.updateVestingScheduleEndDate(superToken, bob, uint32(END_DATE + 1000), EMPTY_CTX);
        //assert storage data
    ...
    function test_updateVestingSchedule_invalidEndDate() public {
        ...
        vm.expectRevert(IVestingSchedulerV3.TimeWindowInvalid.selector);
        vestingScheduler.updateVestingScheduleEndDate(superToken, bob, uint32(initialTimestamp - 1), EMPTY_CTX);
    ...
    function testExecuteCliffAndFlowWithoutCliffAmountOrAdjustment() public {
        ...
        vm.startPrank(alice);
        vestingScheduler.createVestingSchedule(
            superToken, bob, START_DATE, CLIFF_DATE, FLOW_RATE, 0, END_DATE, 0, EMPTY_CTX
        );
        superToken.increaseAllowance(address(vestingScheduler), type(uint256).max);
        vm.stopPrank();
        vm.startPrank(admin);
    ...
    function testExecuteCliffAndFlowWithUpdatedEndDate() public {
        ...
        vestingScheduler.updateVestingScheduleEndDate(superToken, bob, NEW_END_DATE, EMPTY_CTX);

        IVestingSchedulerV3.VestingSchedule memory schedule =
            vestingScheduler.getVestingSchedule(address(superToken), alice, bob);

        vm.warp(schedule.endDate - 1 hours);

        uint256 adjustedAmountClosing = uint96(schedule.flowRate) * 1 hours + schedule.remainderAmount;
        ...
        // Total amount streamed should be the same as the original amount planned (should just stream it faster with updated end date).
        uint256 aliceShouldStream = (END_DATE - CLIFF_DATE) * uint96(FLOW_RATE) + CLIFF_TRANSFER_AMOUNT;

        assertEq(aliceInitialBalance - superToken.balanceOf(alice), aliceShouldStream, "(sender) wrong final balance");
        assertEq(superToken.balanceOf(bob), bobInitialBalance + aliceShouldStream, "(receiver) wrong final balance");
    }
    ...
    function testNewFunctionScheduleCreationWithCliff(uint8 randomizer) public {
        ...
        if (useCtx) {
            vestingScheduler.createVestingScheduleFromAmountAndDuration(
                superToken, bob, totalVestedAmount, vestingDuration, startDate, cliffPeriod, 0, EMPTY_CTX
            );
        } else {
            vestingScheduler.createVestingScheduleFromAmountAndDuration(
    ...
    function test_createScheduleFromAmountAndDuration_executeCliffAndFlow_executeEndVesting_withClaim(
        ...
        if (randomizer % 3 == 0) {
            ...
            vestingScheduler.createVestingScheduleFromAmountAndDuration(
                superToken, bob, totalAmount, totalDuration, startDate, cliffPeriod, claimPeriod, EMPTY_CTX
            );
        }
        vm.stopPrank();
        ...
        public
    {
        ...
        vestingScheduler.createAndExecuteVestingScheduleFromAmountAndDuration(
            superToken, bob, _totalAmount, _totalDuration, EMPTY_CTX
        );
    ...
    function test_createClaimableVestingScheduleFromAmountAndDuration_withCliff(uint8 randomizer) public {
        ...
        if (useCtx) {
            vestingScheduler.createVestingScheduleFromAmountAndDuration(
                superToken, bob, totalVestedAmount, vestingDuration, startDate, cliffPeriod, claimPeriod, EMPTY_CTX
            );
        } else {
            vestingScheduler.createVestingScheduleFromAmountAndDuration(
    ...
    function test_getMaximumNeededTokenAllowance_should_end_with_zero_if_extreme_ranges_are_used(
        ...
        vm.startPrank(alice);
        vestingScheduler.createVestingScheduleFromAmountAndDuration(
            superToken, bob, totalAmount, totalDuration, startDate, cliffPeriod, 0, EMPTY_CTX
        );
        vm.stopPrank();
    ...
    function test_getSender_throws_when_invalid_host() public {
        ...
        vm.startPrank(alice);
        vestingScheduler.createVestingSchedule(
            superToken, bob, START_DATE, CLIFF_DATE, FLOW_RATE, CLIFF_TRANSFER_AMOUNT, END_DATE, 0, NON_EMPTY_CTX
        );
        vm.stopPrank();
    }
